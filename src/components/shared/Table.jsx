          ...
          <thead className="bg-gray-50">
            <tr role="row">
              {columns.map((column, index) => (
                <th key={index} role="columnheader" scope="col" className="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wide">
                  {column.header}
                </th>
              ))}
            </tr>
            ...
            {data.length > 0 ? data.map((row, rowIndex) => (
              ...
              >
                {columns.map((column, colIndex) => (
                  <td key={colIndex} role="cell" className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                    {column.render
                      ? column.render(row)
                      : typeof row[column.accessor] === "object"
                        ? row[column.accessor]?.label || JSON.stringify(row[column.accessor])
                        : row[column.accessor]
                    }
